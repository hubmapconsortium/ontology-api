...
# CodeID
node_metadata['node_id'] = node_metadata['node_id'].str.upper().str.replace('#', ' ').str.replace('_', ' ').str.split('/').str[-1]
...
explode_dbxrefs = node_metadata.explode('node_dbxrefs')[['node_id','node_dbxrefs']].dropna().drop_duplicates().reset_index(drop=True)
...
# changed below to approach with no set function
#def setfunction(x):
#   return set(x)
...
# This does not address two different owl files asserting two different SUIs as PT with the same CUI,CodeID - a Code/CUI could have two PT with two different SUI until that more sophisticated duplicate checking (three column only) is addressed - to address this more generally the schema could assert an SAB on the CODE-SUI relationship which it does not at this time - then no existence check would be required either - does one also consider an SAB on a CUI-CODE relationship? NO! - because it gets messy to prefer terms, one alternative is to assert codes in specific OWL_SAB for each that desires a designated PT - better (best?) is to assert a vocabulary-specific relationship type for vocabulary-specific preferred term (an ingest parameter perhaps) which then would create a PT (if it doesn't have one) and a SAB_PT - yes, that is the solution for an SAB that wants to assert PT on someone else's Code
