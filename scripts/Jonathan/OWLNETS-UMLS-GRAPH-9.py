...
# change field names and matched ones (already exist in SUIs file)
...
# Select for NaN in name to write out
newSUIsOut = newSUIs[['SUI:ID','name']]

# write out justnewSUIs - comment out during development
newSUIsOut.to_csv(csv_path('SUIs.csv'), mode='a', header=False, index=False)

# del newSUIs - not here because we use this dataframe later
# del newSUIsOut - also use this later
...
# This does NOT (yet) address two different owl files asserting two different SUIs as PT with the same CUI,CodeID by choosing the first one in the build process (by comparing only three columns in the existence check) - a Code/CUI would thus have only one PT relationship (to only one SUI) so that is not guaranteed right now (its good practice in query to deduplicate anyway results - because for example even fully addressed two PT relationships could exist between a CODE and SUI if they are asserted on diferent CUIs anyway) - to assert a vocabulary-specific relationship type as vocabulary-specific preferred term (an ingest parameter perhaps) one would create a PT (if it doesn't have one) and a SAB_PT - that is the solution for an SAB that wants to assert PT on someone else's Code (CCF may want this so there could be CCF_PT Terms on UBERON codes) - note that for SY later, this is not an issue because SY are expected to be multiple and so we use all four columns in the existence check there too but intend to keep that one that way
newCODE_SUIs = newSUIs[['SUI:ID','node_id','CUI']]
...
# update the SUIs dataframe to total those now in SUIs.csv
SUIs = pd.concat([SUIs,newSUIsOut], axis=0).reset_index(drop=True)
...
# del newSUIs - still using this (updated to SYs now) again later
del explode_syns
del newSUIsOut
...
# #### Write DEFs (ATUI:ID, SAB, DEF) and DEFrel (:END_ID, :START_ID) - with existence check
...
df = newDEF_REL.merge(DEF_REL, on=DEF_REL.columns.to_list(), how='left', indicator=True)
...
# ### Backlog items:
# #### Extract and write DEFs and CUI-DEFs from OWLNETS in node_metadata as node_definision column
