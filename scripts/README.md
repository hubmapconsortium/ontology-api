...
The .csv files used to build the Neo4j database are built by running the 'scripts/build_csv.sh' script.
The ontologies to be processed are expressed on the command line like the following which will process the
fifteen ontologies specified.
```
$ cd scripts
$ ./build_csv.sh -v PATO UBERON CL DOID CCFASCTB OBI EDAM HSAPDV SBO MI CHEBI MP ORDO PR UNIPROTKB
```
The approximate run time for creating the .csv files associated with these 15 ontologies is about 26 hrs on a MacBook Pro 2.6 GHz 6-core I7 /w 32 GB 2667 MHz memory.
...
The './owlnets_script/__main__.py' is used to convert the Owls to OelNets (tab delimited files).
...
This script takes it's input from the './owlnets_output/' files, and a base set of [UMLS](https://www.nlm.nih.gov/research/umls/index.html) (.csv files) files.
It writes to the '../neo4j/import/current' directory.
It's changes to the files in that directory are cumulative between runs, so when starting a run a freshly downloaded set of UMLS files should be placed there.
